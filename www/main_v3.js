...
//		me.tt.updateInterface($('#dropdownUploadsLi')) ;
	...
	loadWikidataLayer : function () {
		...
		sparql += 'SELECT ?q ?qLabel ?location ?image ?reason ?desc ?commonscat WHERE { ' ;
		if ( me.worldwide ) {
	...
	updateLayers : function () {
		...
		$.each ( ['commons','wikipedia','wikidata','flickr'] , function ( k , v ) {
			me.loadLayer ( v ) ;
		} ) ;
	} ,
	...
	init : function () {
		...
		function fin () {
			...
			alert ( (wsm_comm.isLoggedIn('wikidata')?'L':'Not l')+'ogged into wikidata' ) ;
			alert ( (wsm_comm.isLoggedIn('commons')?'L':'Not l')+'ogged into commons' ) ;
